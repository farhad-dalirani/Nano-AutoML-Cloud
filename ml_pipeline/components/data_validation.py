...
class DataValidation:
    def __init__(
        ...
        try:
            ...
            self._schema_config = read_schema_file(
                schema_filepath=self.training_pipeline_config.schema_file_path
            )
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        ...
        try:
            ...
            logging.info(
                "Required number of columns: {}".format(num_of_columns_in_schema)
            )
    ...
    def validate_target_column(self, dataframe: pd.DataFrame) -> bool:
        ...
        try:
            if (
                "target_column" not in self._schema_config
                or not self._schema_config["target_column"]
            ):
                raise ValueError("Missing or empty 'target_column' in schema config.")

            if (
                "task_type" not in self._schema_config
                or not self._schema_config["task_type"]
            ):
            ...
            if target_column not in dataframe.columns:
                logging.error(
                    "Target column '%s' not found in DataFrame.", target_column
                )
                return False
            ...
            if target_data.isnull().any():
                logging.warning(
                    "Target column '%s' contains null values.", target_column
                )
                return False
            ...
            if task_type == "classification":
                ...
                if target_data.dtype == object or num_unique < 100:
                    ...
                    logging.warning(
                        "Target column '%s' may not be categorical for classification task.",
                        target_column,
                    )
                    return False
                ...
                if pd.api.types.is_numeric_dtype(target_data):
                    ...
                    logging.warning(
                        "Target column '%s' is not numeric for regression task.",
                        target_column,
                    )
                    return False
                ...
                logging.error(
                    "Unknown task type in schema: %s, should be 'regression' or 'classification'.",
                    task_type,
                )
                return False
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            status_num_col_train = self.validate_number_of_columns(
                dataframe=train_dataframe
            )
            status_num_col_test = self.validate_number_of_columns(
                dataframe=test_dataframe
            )

            # Validate target column
            status_target_col_train = self.validate_target_column(
                dataframe=train_dataframe
            )
            status_target_col_test = self.validate_target_column(
                dataframe=test_dataframe
            )
            ...
            overal_status = (
                status_drift
                and status_num_col_train
                and status_num_col_test
                and status_target_col_train
                and status_target_col_test
            )
